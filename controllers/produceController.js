...
// Route usage: router.post('/products', upload.single("product_Photo"), addProduct);
...
photo = newPath.replace(/\\/g, "/"); // Normalize slashes
}
...
unit,
pricePerUnit,
product_Photo: photo, // Save uploaded photo path
county,
subCounty,
village,
});
...
return res
.status(500)
.json({ message: "Error adding the product", error: error.message });
}
};

// Export multer upload for route use
exports.uploadProductPhoto = upload.single("product_Photo");
